...
def get_current_branch_and_commit():
    ...
    if os.path.exists(head_path):
        with open(head_path, "r") as head_file:
            ...
            commit_hash = lines[1].strip() if len(lines) > 1 else None
...
def get_commit_history(commit_hash):
    ...
    while commit_hash:
        ...
        with open(commit_path, "r") as commit_file:
            ...
            commit_hash = commit_data.get("parent_commit")  # Move to parent commit
...
def view_commit_history():
    ...
    try:
        ...
        if not current_branch or not latest_commit_hash:
            console.print("[bold red]Error: No valid branch or commit found in HEAD.[/bold red]")
            return
